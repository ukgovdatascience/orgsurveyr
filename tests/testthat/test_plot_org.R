...
test_that('plot_org generates expected output', {
  ...
  expect_is(plot_org(tg_org, fill_var = 'org_depth', df=NULL, is_circular = FALSE), 'ggplot')
...
test_that('plot_org input parameter error handling works',{
  ...
  expect_error(plot_org(tg_org, fill_var='org_depth', df = 1, is_circular = FALSE),
               'inherits')

  expect_error(plot_org(tg_org, fill_var='org_depth', df = data_frame(), is_circular = FALSE),
               'data frame should be in org_tall_df format')

  expect_error(plot_org(tg_org, fill_var=c(1,2), df = NULL, is_circular = FALSE),
               'character')

  expect_error(plot_org(tg_org, fill_var='org_depth', df = NULL, is_circular = 3),
               'logical')
